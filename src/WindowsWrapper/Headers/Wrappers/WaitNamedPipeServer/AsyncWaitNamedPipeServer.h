...
{
	class AsyncWaitNamedPipeServer final
	{
	public:
		explicit AsyncWaitNamedPipeServer(const std::wstring& pipe_name,
		                                  uint32_t open_mode = Consts::PIPE_DEFAULT_MODE | FILE_FLAG_OVERLAPPED,
		...
		[[nodiscard]] Overlapped connect_named_pipe() const;
		[[nodiscard]] bool is_pipe_connected() const;
		void disconnect_named_pipe() const;

		[[nodiscard]] Overlapped write_named_pipe(const std::vector<std::byte>& buffer) const;
		[[nodiscard]] Overlapped read_named_pipe(uint32_t number_of_bytes_to_read) const;
